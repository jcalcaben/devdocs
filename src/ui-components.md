...
redirect_from: /guides/v2.3/ui_comp_guide/bk-ui_comps.html
---

## Overview of UI components

_Magento UI components are used to represent distinct UI elements, such as tables, buttons, dialogs, and others_.
...
To use UI components in your custom module, you need to add a dependency for the Magento_UI module in [your component's composer.json file]({{page.baseurl}}/extension-development/files/composer-json.html).
...
Basic components are declared in the [page layout files]({{page.baseurl}}/frontend-development/layouts/file-types.html#layout-types-page); secondary components are declared in the top-level components’ instances configuration files.
...
UI components work well together: they communicate with each other via the [uiRegistry service]({{page.baseurl}}/ui-components/troubleshoot.html#debugging-using-the-uiregistry) that tracks their asynchronous initialization. Therefore, if we need to extend something that has already been implemented as a hierarchy of UI components or add a new feature that should interact with other UI components, it's easier and more effective to use a UI component.
...
2. **JavaScript** class inherited from one of the Magento JavaScript framework UI components base classes (such as [UIElement]({{page.baseurl}}/ui-components/concepts/uielement-class.html), [UIClass]({{page.baseurl}}/ui-components/concepts/uiclass-library.html) or [UICollection]({{page.baseurl}}/ui-components/concepts/uicollection-class.html)).
...
![JavaScript class implementation of a UI component]({{site.baseurl}}/static/images/ui_comp_js_class.png)
...
2. [UI component's XML declaration]({{page.baseurl}}/ui-components/concepts/xml-configuration.html).
3. [Backend/PHP modifiers]({{page.baseurl}}/ui-components/concepts/php-modifiers.html).
