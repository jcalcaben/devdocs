...
redirect_from: /guides/v2.3/ui_comp_guide/concepts/ui_comp_uicollection_concept.html
---

## What is `uiCollection`

The `uiCollection` library class should be used as a base class by any components that contain a collection of child UI components.  `uiCollection` inherits from the [uiElement class]({{page.baseurl}}/ui-components/concepts/uielement-class.html).
...
* The `destroy()` method removes the following for the child components and itself:
  		_ link to the component in `uiRegistry`
  		_ link to the component in the parent component
  		\* event listeners
...
* `childDefaults` can be used to set the children defaults: properties from `childDefaults` are set into child elements' [`defaults` property]({{page.baseurl}}/ui-components/concepts/uiclass-library.html#uiclass_properties).
