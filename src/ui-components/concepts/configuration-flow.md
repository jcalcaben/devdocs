...
redirect_from: /guides/v2.3/ui_comp_guide/concepts/ui_comp_config_flow_concept.html
---
...
When the server generates a page response, the configuration of these components in the [`.xml` declaration files]({{page.baseurl}}/ui-components/concepts/xml-configuration.html) is then modified by the [`.php` modifiers]({{page.baseurl}}/ui-components/concepts/php-modifiers.html), and then finally this combined configuration is packed into JSON format and added into the HTTP response body.
...
1. RequireJS requires `Magento_Ui/js/core/app` and passes [JSON configuration]({{page.baseurl}}/javascript-development/core-concepts/script-initialize-call.html#declarative-notation-using-the-script-typetextx-magento-init--tag-decl_tag) as an parameter.
