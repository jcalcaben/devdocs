...
functional_areas:
  - Checkout
redirect_from: /guides/v2.3/howdoi/checkout/checkout_new_field.html
---
...
Add the field to layout. Both shipping address and billing address forms are [generated dynamically]({{page.baseurl}}/ui-components/checkout-tutorial/add-input-form.html#dynamic_form). So to modify its layout, you need to create a [plugin]({{page.baseurl}}/extension-development/class-types/plugin.html) for the `\Magento\Checkout\Block\Checkout\LayoutProcessor::process` method.
...
Optionally, instead of adding a plugin, you can use [dependency injection (DI)]({{page.baseurl}}/extension-development/core-concepts/dependency-injection.html). For this, in `<your_module_dir>/Block/Checkout/` directory, add the `LayoutProcessor` class adding the custom field to the address form. The class must implement the `\Magento\Checkout\Block\Checkout\LayoutProcessorInterface` interface. You can use the code sample above as an example of the `\Magento\Checkout\Block\Checkout\LayoutProcessorInterface::process()` method implementation.
...
The following is a sample `extension_attributes.xml`:
...
Now clear the `var/generation` directory when you run `setup:di:compile` command, new getter and setter methods will be added in `/var/generation/Magento/Quote/Api/Data/AddressInterface.php`
...
Now you can set/get these attributes value by creating the instance of `Magento/Quote/Api/Data/AddressInterface.php interface`.
...
* [EAV and extension attributes]({{page.baseurl}}/extension-development/core-concepts/attributes.html)
