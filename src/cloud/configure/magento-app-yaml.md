...
group: cloud-guide
title: Application
redirect_from: /guides/v2.3/cloud/project/project-conf-files_magento-app.html
functional_areas:
  - Cloud
  - Setup
---
...
The name is used in the [`routes.yaml`]({{page.baseurl}}/cloud/configure/routes-yaml.html) file to define the HTTP upstream (by default, `php:http`). For example, if the value of `name` is `app`, you must use `app:http` in the upstream field. You can also use this name in multi-application relationships.
...
See [Services]({{page.baseurl}}/cloud/configure/services-yaml.html) for a full list of currently supported service types and endpoints.
...
Contrary to standard `.htaccess` approaches, which accept a _blacklist_ and allow everything to be accessed except a specific list, we accept a _whitelist_, which means that anything not matched will trigger a 404 error and will be passed through to your `passthru` URL.
...
Describes processes that are triggered on a schedule. We recommend you run `cron` as the [Magento file system owner]({{page.baseurl}}/cloud/setup/setup-magento-file-system-owner.html). Do _not_ run cron as `root` or as the web server user.
...
{:.bs-callout .bs-callout-info}
{{site.data.var.ece}} supports PHP 7.1 and later. For Pro projects **created before October 23, 2017**, you must open a [support ticket]({{page.baseurl}}/cloud/troubleshooting/troubleshooting-intro.html) to use PHP 7.1 on your Pro Staging and Production environments.
...
For a list of recommended PHP configuration settings, see [Required PHP settings]({{page.baseurl}}/install/getting-started/php/settings.html).

After pushing your file, you can check that the custom PHP configuration has been added to your environment by [creating an SSH tunnel]({{page.baseurl}}/cloud/env/environment-overview.html#env-start-tunn) to your environment and entering:
