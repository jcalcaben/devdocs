...
functional_areas:
  - Cloud
  - Setup
redirect_from: /guides/v2.3/cloud/project/project-routes-more-redir.html
---
...
In the [`.magento/routes.yaml`]({{page.baseurl}}/cloud/configure/routes-yaml.html) file you can also add partial {% glossarytooltip 510de766-1ebd-4546-bf38-c618c9c945d2 %}redirect{% endglossarytooltip %} rules
to existing routes:
...
Determines if the suffix is carried over with the redirect. Defaults to `true`, but not supported if `regexp` is `true` _or_ if `prefix` is `false`.
