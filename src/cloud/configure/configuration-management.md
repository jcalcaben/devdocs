...
functional_areas:
  - Cloud
  - Deploy
redirect_from: /guides/v2.3/cloud/live/sens-data-over.html
---
...
* Less time required to [build](#cloud-confman-scd-over) and deploy your project by moving static file deployment from deploy to the build phase. Your site is in maintenance mode until deployment completes. For details, see [Deployment Process]({{page.baseurl}}/cloud/deploy/deployment-process.html).
...
To complete configuration management tasks, you must have a project reader role with [environment administrator]({{page.baseurl}}/cloud/project/project-users.html#cloud-role-env) privileges.
...
For a list of configurable settings, see [Configuration settings you can change](#cloud-clp-settings) and [System settings reference]({{page.baseurl}}/configure/deployment/reference/environment-variables.html).

### Static content deployment performance {#cloud-confman-scd-over}

Depending on the size of your store, you may have a large amount of static content files to deploy. Normally, static content deploys during the [deploy phase]({{page.baseurl}}/cloud/deploy/deployment-process.html#cloud-deploy-over-phases-hook), which is in Maintenance mode. To move the deployment of static content to the [build phase]({{page.baseurl}}/cloud/deploy/deployment-process.html#cloud-deploy-over-phases-build), generate the configuration file.
...
To fully understand the process, see [our extensive example]({{page.baseurl}}/cloud/configure/configuration-example.html).

The **Starter plan** environment high-level overview of this process:

![Overview of Starter configuration management]({{site.baseurl}}/static/images/cloud_configmgmt-starter-2-2.png)

The **Pro plan** environment high-level overview of this process:

![Overview of Pro configuration management]({{site.baseurl}}/static/images/cloud_configmgmt-pro-2-2.png)
...
2. Add the necessary locales using your Admin panel.
3. Generate the `app/etc/config.php` file containing all locales.

   ```bash
   php ./vendor/bin/ece-tools config:dump
   ```

4. Copy the new configuration file from your Integration environment to your local Staging environment directory.

   ```bash
   rsync <SSH URL>:app/etc/config.php ./app/etc/config.php
   ```

5. Push code changes to the remote.
