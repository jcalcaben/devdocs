...
functional_areas:
  ...
  - Setup
  - Services
redirect_from: /guides/v2.3/cloud/live/paypal-onboarding.html
---
...
![Log in to PayPal]({{site.baseurl}}/static/images/cloud_paypal-access.png)
...
3. For PayPal, select **Configure**. Configuration fields display in expandable sections for Express Checkout, Advertise PayPal Credit, and Basic and Advanced settings.
...
5. Configure the Express Checkout settings to authenticate and use the PayPal API:
   ...
   * **API Uses Proxy** select Yes or No to set if the system uses a proxy server to establish a connection between Magento and the PayPal payment system. If Yes, enter the proxy host and port.
