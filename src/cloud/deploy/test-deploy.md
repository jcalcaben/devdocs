...
functional_areas:
  ...
  - Testing
  - Deploy
redirect_from: /guides/v2.3/cloud/live/stage-prod-test.html
---

#### Previous step:

[Migrate data and static files]({{page.baseurl}}/cloud/deploy/migrate-files-data.html)

When your code, files, and data is successfully migrated to Staging or Production, use the environment URLs to test your site(s) and store(s). For a list of your URLs, see [Starter]({{page.baseurl}}/cloud/deploy/prepare-deploy.html#starter-urls) and [Pro]({{page.baseurl}}/cloud/deploy/prepare-deploy.html#pro-urls) access information.
...
For more information, see [View logs for troubleshooting]({{page.baseurl}}/cloud/troubleshooting/environments-logs.html)
...
Verify Fastly is caching properly on Staging and Production. [Configuring Fastly]({{page.baseurl}}/cloud/configure/configure-fastly.html) requires careful attention to details, using the correct Fastly Service ID and Fastly API key, and a proper VCL snippet uploaded.
...
For Starter, enter the full site URL from your environment [Access info]({{page.baseurl}}/cloud/deploy/prepare-deploy.html#starter-urls) in the command to view the headers.
...
* [Blackfire]({{page.baseurl}}/cloud/configure/blackfire.html) (provided): Helps track through the issues New Relic finds and helps you dig deeper into the issue for specifics. Blackfire profiles the environment and helps locate bottlenecks indepth: process, method call, query, load, and so on.
...
We provide a free Security Scan Tool for your sites. To add your sites and run the tool, see [Magento Security Scan Tool]({{page.baseurl}}/cloud/live/go-live-overview.html#security-scan).
