...
functional_areas:
  - Cloud
  - Configuration
redirect_from: /guides/v2.3/cloud/env/variables-deploy.html
---
...
Enables or disables cleaning [static content files]({{page.baseurl}}/configure/command-line/static-content/deploy.html#config-cli-static-overview) generated during the build or deploy phase. We recommend the default value _true_ in development.
...
deploy updates to existing files without removing the previous versions. Because of [static file fallback]({{page.baseurl}}/frontend-development/cache.html#clean_static_cache) rules, fallback operations can display the wrong file if the directory contains multiple versions of the same file.
...
* `consumers`—An array of strings specifying which consumer(s) to run. An empty array runs _all_ consumers. Refer to [List consumers]({{page.baseurl}}/configure/message-queues/manage-queues.html#list-consumers) for more information.
...
By default, the deployment process overwrites all settings in the `env.php` file. Refer to [Manage message queues]({{page.baseurl}}/configure/message-queues/manage-queues.html) for more information about how this works in {{site.data.var.ce}} and {{site.data.var.ee}}.
