...
redirect_from: /guides/v2.3/payments-integrations/payment-gateway/command-pool.html
---

## Gateway command pool

All [gateway commands]({{page.baseurl}}/payment-integration/gateway/command.html) implemented for a particular payment provider, should be added to a command pool for this provider. A command pool is a set of gateway commands available for integration with a particular payment provider. The pool is added to the configuration of the payment provider using [dependency injection]({{page.baseurl}}/extension-development/core-concepts/dependency-injection.html).
