...
title: Page
redirect_from: /guides/v2.3/mtf/mtf_entities/mtf_page.html
---

A page object is a class that serves to interact with the Magento page under test.
A page serves as container for [blocks]({{page.baseurl}}/testing/functional-testing-framework/entities/page.html).
...
Also, block can contain a `render` node. [Read about renders in the Block topic]({{page.baseurl}}/testing/functional-testing-framework/entities/block.html#mtf_block_render).
...
Storefront page is recognizable by `area` assigned any value except `Adminhtml`, _and_ `mca` doesn't have `http`. This type of page extends class [Magento\Mtf\Page\FrontendPage][].
...
External page has `area` assigned any value except `Adminhtml`, _and_ `mca` containing `http`. Generated page extends class [Magento\Mtf\Page\ExternalPage][].
