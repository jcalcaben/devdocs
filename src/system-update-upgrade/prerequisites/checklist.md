...
functional_areas:
  - Upgrade
redirect_from: /guides/v2.3/comp-mgr/prereq/prereq_compman-checklist.html
---

Before you continue, to avoid errors during your installation or update, make sure you verify _all_ of the following:
...
For details, see [Set up cron]({{page.baseurl}}/install/post-install/configuration.html#post-install-cron).

### File system permissions {#perms}

For security reasons, Magento requires certain permissions on the file system. Permissions are different from [_ownership_](#magento-owner-group).
Ownership determines _who_ can perform actions on the file system; permissions determine _what_ the user can do.

Directories in the Magento file system must be writable by the [Magento file system owner's]({{page.baseurl}}/install/getting-started/file-system-ownership-permissions.html) group.
...
For more information, see [File system permissions and ownership]({{page.baseurl}}/install/getting-started/file-system-ownership-permissions.html).
