...
functional_areas:
  ...
  - System
  - Setup
redirect_from: /guides/v2.3/config-guide/cli/config-cli-subcommands-static-view.html
---
...
* **[Default]({{page.baseurl}}/configure/application-initialization/magento-modes.html#default-mode)** and **[developer]({{page.baseurl}}/configure/application-initialization/magento-modes.html#developer-mode)** modes: Magento generates them on demand, but the rest are cached in a file for speed of access.
* **[Production]({{page.baseurl}}/configure/application-initialization/magento-modes.html#production-mode)** mode: Static files are _not_ generated or cached.
    ...
    <tr>
      ...
      <td>
        Define the deployment strategy. Use these options only if you have more than one locale.
				<ul>
					<li>
	          Use the [quick strategy]({{page.baseurl}}/config-guide/cli/config-cli-subcommands-static-deploy-strategies.html#static-file-quick) to minimize deployment time. This is the default command option if not specified.
					</li>
					<li>
						Use the [standard strategy]({{page.baseurl}}/config-guide/cli/config-cli-subcommands-static-deploy-strategies.html#static-file-standard) to deploy all static view files for all packages.
					</li>
					<li>
						Use the [compact strategy]({{page.baseurl}}/config-guide/cli/config-cli-subcommands-static-deploy-strategies.html#static-file-compact) to conserve disk space on the server.
					</li>
				</ul>
      </td>
      <td>
...
1. Run [`bin/magento app:config:dump`]({{page.baseurl}}/configure/command-line/export-configuration.html) to export the configuration from your production system.
...
[Install the Magento software first]({{page.baseurl}}/install/getting-started.html); otherwise, you cannot run the static view files deployment tool.
...
2. Log in to the Magento server as, or [switch to]({{page.baseurl}}/install/getting-started/file-system-ownership-permissions.html), the Magento file system owner.
