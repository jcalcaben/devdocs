...
To ensure that your new string is added to the dictionary and translated, use the `__('<your_string>')` method when outputting a string in a [.phtml template]({{page.baseurl}}/frontend-development/templates.html).
...
If your theme contains [custom email templates]({{page.baseurl}}/frontend-development/templates/email.html#customize-email-theme), their strings can be added to the dictionary as well.
To make sure the strings of an email template are added to the dictionary, use the `{{trans}}` [directive]({{page.baseurl}}/frontend-development/templates/email.html#localization).

Custom email templates [added using the Admin panel]({{page.baseurl}}/frontend-development/templates/email.html#customize-email-admin), are not stored in the file system, and their strings are not added to the dictionary.
...
In this example, you would use a call similar to the following to display the translated _Delete_ string.
...
[dictionary]: {{page.baseurl}}/configure/command-line/localization.html#config-cli-subcommands-xlate-dict
