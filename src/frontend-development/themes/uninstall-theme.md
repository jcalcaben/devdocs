...
functional_areas:
  - Frontend
  - Theme
redirect_from: /guides/v2.3/frontend-dev-guide/themes/theme-uninstall.html
---
...
* the way Magento was installed: [using the source files from GitHub]({{page.baseurl}}/install/command-line/git-sample-data.html) or [using Composer]({{page.baseurl}}/install/command-line/composer-sample-data.html). 

The following sections describe the flow for uninstalling themes in each case.

## Prerequisites

1. [Set your Magento application to the developer or default mode]({{page.baseurl}}/configure/command-line/set-magento-mode.html).
...
3. Make sure that the theme is not defined as a parent for any registered theme. To do this, in the Admin panel, navigate to **Content** > **Design** > **Themes**. Make sure that your theme is not mentioned in the **Parent Theme** column. If it is mentioned, you need to uninstall the child theme first. 
...
If both the theme and the Magento instance were installed using Composer, you can use a special CLI command. Follow the instructions from the [Uninstall themes Composer package]({{page.baseurl}}/install/command-line/uninstall-theme.html) topic.
...
4. Use the `magento theme:uninstall` CLI command as described in the [Uninstall themes Composer package]({{page.baseurl}}/install/command-line/uninstall-theme.html) topic.
