...
redirect_from: /guides/v2.3/frontend-dev-guide/themes/theme-create.html
functional_areas:
  - Frontend
  - Theme
---
...
A new theme you create is not applied for your store automatically. You need to apply it manually in the Admin panel. This procedure is described in the [Apply and configure a theme in Admin]({{page.baseurl}}/frontend-development/themes/apply-theme.html) topic.
...
6. Configure your theme in the {% glossarytooltip 29ddb393-ca22-4df9-a8d4-0024d75739b1 %}Admin{% endglossarytooltip %} panel.

## Recommended reading

* [Checklist of modules]({{site.mage2300url}}app/code/Magento)
* [Static view files processing]({{page.baseurl}}/configure/command-line/static-content/deploy.html)
...
After you create a directory for your theme, you must create `theme.xml` containing at least the theme name and the parent theme name (if the theme [inherits]({{page.baseurl}}/frontend-development/themes/set-inheritance.html) from one). Optionally you can specify where the theme preview image is stored.
...
You can find details about the Composer integration in the Magento system in [Composer integration]({{page.baseurl}}/extension-development/files/composer-json.html).
...
For illustration, see the [registration.php]({{site.mage2300url}}app/design/frontend/Magento/luma/registration.php) file in the Magento Luma theme.
...
1. Log in to your Magento server as a user with permissions to create directories and files in the Magento installation directory. (Typically, this is the [Magento file system owner]({{page.baseurl}}/install/getting-started/apache.html).)

2. Create the `etc` directory in your theme folder.

3. Copy the `view.xml` file from the `etc` directory of an existing theme (for example, from the Blank theme) to your theme's `etc` directory.

4. Configure all storefront product image sizes in the `view.xml` file. For example, you can make the {% glossarytooltip 50e49338-1e6c-4473-8527-9e401d67ea2b %}category{% endglossarytooltip %} grid view product images square by specifying a size of 250 x 250 pixels:
...
For details about images configuration in the `view.xml` file, see the [Configure images properties for a theme]({{page.baseurl}}/frontend-development/themes/configure-images.html) topic.
...
To declare a theme logo, add an [extending]({{page.baseurl}}/frontend-development/layouts/extend.html) `<theme_dir>/Magento_Theme/layout/default.xml` layout.
...
To learn more about theme layouts, refer to the [Layout section]({{page.baseurl}}/frontend-development/layouts.html) of this guide.
...
For information on how to apply the theme for the storefront, see the [Apply and configure a theme in Admin]({{page.baseurl}}/frontend-development/themes/apply-theme.html) topic.
