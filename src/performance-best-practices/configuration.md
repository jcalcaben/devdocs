...
functional_areas:
  ...
  - System
  - Setup
redirect_from: /guides/v2.3/performance-best-practices/configuration.html
---
...
All asynchronous operations in {{site.data.var.ce}} are performed using the Linux `cron` command. See [Configure and run cron]({{page.baseurl}}/configure/command-line/cron.html) to configure it correctly.
...
See the [Reindex in parallel mode section]({{page.baseurl}}/configure/command-line/manage-indexers.html#config-cli-subcommands-index-reindex-parallel) of [Manage the indexers]({{page.baseurl}}/configure/command-line/manage-indexers.html) for more information.
...
The **Developer** tab and options are only available in [Developer mode]({{page.baseurl}}/configure/command-line/set-magento-mode.html).
